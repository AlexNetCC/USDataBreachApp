...
const AskAI: React.FC<AskAIProps> = ({ law }) => {
  ...
  return (
    <div className="bg-brand-light border border-brand-accent/50 rounded-lg">
      <button
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <span className="text-lg font-semibold font-display text-brand-primary">Ask AI About {law.state}'s Law</span>
        </div>
        <svg
        ...
        <div className="p-4 border-t border-brand-accent/50">
          ...
          <form onSubmit={handleSubmit} className="flex items-center space-x-2">
            <input
              ...
              className="flex-grow p-2 border border-border-color rounded-md focus:ring-2 focus:ring-brand-accent bg-white text-gray-900"
              disabled={isLoading}
            />
            <button
              type="submit"
          ...
          {(isLoading || response || error) && (
            <div className="mt-4 p-4 bg-white rounded-md border border-border-color max-h-60 overflow-y-auto">
