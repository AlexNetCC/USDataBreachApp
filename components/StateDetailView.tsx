  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <header className="pb-4 border-b border-border-light">
        <h1 className="text-5xl font-bold font-display text-text-primary">{law.state}</h1>
        <p className="mt-2 text-lg text-text-secondary">{law.statuteTitle}</p>
        <a href={law.statuteUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-accent hover:underline">
          {law.statuteCitation}
        </a>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <InfoCard title="Individual Notification Timeline">
            <p className="text-3xl font-bold text-accent">{law.individualNotificationTimelineDays > 0 ? `${law.individualNotificationTimelineDays} days` : 'ASAP'}</p>
        </InfoCard>
      ...
      <InfoCard title="Breach Definition" defaultOpen>
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-3 bg-gray-50 rounded-md border border-border-light">
                <p className="text-sm font-medium text-text-secondary">Trigger</p>
                <p className="font-semibold text-text-primary">{law.breachTrigger.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}</p>
            </div>
            <div className="p-3 bg-gray-50 rounded-md border border-border-light">
        ...
        <div className="space-y-3">
          ...
          {law.riskOfHarmAnalysisCanEliminateNotification && (
            <div>
              ...
              <blockquote className="pl-4 border-l-4 border-border-light italic text-text-secondary">{law.riskOfHarmAnalysisStandard}</blockquote>
            </div>
          )}
        </div>
      </InfoCard>

      <div className="bg-surface-light p-6 rounded-lg shadow-sm border border-border-light">
